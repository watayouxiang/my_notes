...
假如没有class，那么每个 “对象” 都有一份相同的 “类信息”，就会造成内存爆棚。假如没有 “对象”，虽然节省了 “内存” ，但是每一个对象都能通过修改公共信息，从而改变其他对象的值（公共信息存在class里，差异信息存在object里）。
...
java语言最大的优势是不用程序员管理内存，Oracle为了管理java对象生命周期，设计了方法区和堆区只是为了好管理。
