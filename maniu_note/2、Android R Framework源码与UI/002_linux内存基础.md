...
如果处理器启用了MMU，CPU执行单元发出的内存地址将被MMU截获，从CPU到MMU的地址称为虚拟地址VA，而MMU将这个地址翻译成另一个地址发到CPU芯片的外部地址引脚上，也就是将VA映射成PA，如下图：
...
虚拟内存与交换区的这种概念，实现了大内存需求量的（多个）进程，能够（同时）运行在较小的物理内存中。如下图所示：
...
- 地址范围
  ...
  - 32位操作系统的地址范围是2的32次方，也就是4G。
  - 32位操作系统没办法访问超过4G的内存地址。

- 内核空间、用户空间

  - 比如一个4G内存的操作系统，1G内存空间分配给操作系统作为内核空间，3G作为用户空间
  - 内核空间运行优先级较高的代码：如内核代码、驱动代码
  - 区分内核空间和用户空间的好处是：应用程序的崩溃不会造成系统的崩溃

- 分页机制

  - 页、页帧（页框）
    - 为了cpu的高效执行，并方便管理内存。cpu取一次数据，大小为一页（一般为 4K 或 8K）
    - 物理地址中的取一次数据的单位大小称为 ”页帧“，虚拟地址中取一次数据的单位大小称为 ”页“
    - “页帧” 也叫 “页框”
  
  - 物理页、虚拟页
  
    - 一页在物理内存中称为 “物理页”，在虚拟内存中称为 “虚拟页”
  
  - 页表
    - “页表” 是 “物理页” 和 “虚拟页” 的映射表
    - MMU 通过 “页表” 实现 “物理地址” 和 “虚拟地址” 的相互转换
  
  - MMU
    - MMU是内存管理单元的简称，用来管理 “虚拟存储器” 和 “物理存储器”
    - MMU可以将 “虚拟地址” 和 “物理地址” 进行互相转换
  
  - 交换区
  
    - 根据程序的局部性原则，在很小一段时间内，只有很少一部分代码会被cpu执行
    - 交换区是一块磁盘物理空间，是代码的缓存区
    - 当进行虚拟地址和物理地址的映射时，会先去交换区中查找运行所需的代码，如果没有，才会去获取物理地址中获取代码，并将该代码放入交换区中。
